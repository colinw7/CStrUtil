...
toWords(const std::string &str, CWordDef *def)
{
  ...
  while (i < len && def->isSpace(str[i]))
    ++i;

  while (i < len) {
    CStrWord word = readWord(str, &i, def);
...
void
CStrUtil::
toWords(const std::string &str, std::vector<std::string> &words, CWordDef *def)
{
  words.clear();

  addWords(str, words, def);
}
...
void
CStrUtil::
toIntegers(const std::string &str, std::vector<int> &integers, const std::string &splitters)
{
  integers.clear();
...
void
CStrUtil::
toWords(const std::string &str, std::vector<std::string> &words, const std::string &splitters)
{
  words.clear();
...
readWord(const std::string &str, uint *pos, CWordDef *def)
{
  ...
  if      (def->isStartGroupI(str[*pos])) {
    ...
    while (*pos < len) {
      ...
      else if (def->isEndGroupI(str[*pos])) {
        word.setEndGroup(str[*pos]);
...
CStrWords
CStrUtil::
toFields(const std::string &str, const std::string &splitters, bool skipEmpty)
{
  CStrWords words(str);
...
getIsCType(const std::string &is_type)
{
  ...
  else if (CStrUtil::casecmp(is_type, "ascii") == 0)
    return CStrUtil::isascii;
...
/* isascii for machines which only have macro */
...
vsprintf(std::string &str, const char *format, va_list *vargs)
{
  class PrintF : public CPrintF {
   ...
   public:
    PrintF(const char *format, va_list *vargs) :
